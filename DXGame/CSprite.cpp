...
void CSprite::DrawCenter(D2D1_POINT_2F Pos)
{
	m_gfx->GetRenderTarget()->SetTransform(D2D1::Matrix3x2F::Identity());

	FLOAT Width = m_bmp->GetSize().width / 2;
	FLOAT Height = m_bmp->GetSize().height / 2;

	m_gfx->GetRenderTarget()->DrawBitmap(
		m_bmp,
		D2D1::RectF(Pos.x - Width, Pos.y - Height,
...
void CSprite::DrawCenter(D2D1_RECT_F* src, D2D1_POINT_2F pos, D2D1_POINT_2F Scale, D2D1_POINT_2F* center, float angle)
{
	m_gfx->GetRenderTarget()->SetTransform(D2D1::Matrix3x2F::Identity());

	FLOAT Width = (m_sprWidth * Scale.x) / 2;
	FLOAT Height = (m_sprHeight * Scale.y) / 2;
